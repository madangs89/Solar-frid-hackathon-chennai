...
function avgFromHistory(history) {
  if (!history || history.length === 0) return { power: 0, efficiency: 0, irradiance: 0 };
  const len = history.length;
  const power      = history.reduce((s, r) => s + Number(r.power      || 0), 0) / len;
  const efficiency = history.reduce((s, r) => s + Number(r.efficiency || 0), 0) / len;
  const irradiance = history.reduce((s, r) => s + Number(r.irradiance || 0), 0) / len;
...
function SolarCellGrid({ status }) {
  ...
  return (
    <svg viewBox="0 0 80 52" className="w-full h-full" style={{ display: "block" }}>
      <defs>
        <linearGradient id={`cellGrad-${status}`} x1="0%" y1="0%" x2="100%" y2="100%">
          <stop offset="0%"   stopColor={s.glow} stopOpacity="0.18" />
          <stop offset="100%" stopColor={s.glow} stopOpacity="0.04" />
        </linearGradient>
      </defs>
      <rect x="1" y="1" width="78" height="50" rx="3"
        fill={`url(#cellGrad-${status})`} stroke={s.cellLine} strokeWidth="0.8" />
      {[18, 35, 52].map((y) => (
        <line key={y} x1="1" y1={y} x2="79" y2={y} stroke={s.cellLine} strokeWidth="0.6" />
      ))}
      {[20, 40, 60].map((x) => (
        <line key={x} x1={x} y1="1" x2={x} y2="51" stroke={s.cellLine} strokeWidth="0.6" />
      ))}
      <line x1="1"  y1="1"  x2="79" y2="51" stroke={s.glow} strokeWidth="0.4" strokeOpacity="0.15" />
      <line x1="79" y1="1"  x2="1"  y2="51" stroke={s.glow} strokeWidth="0.4" strokeOpacity="0.10" />
  ...
  if (!lat || !lng) {
    return (
      <div style={{
        ...
        <span style={{ fontSize: 7, color: "rgba(255,255,255,0.15)", letterSpacing: "0.1em" }}>
          NO LOCATION
        </span>
      </div>
    );
...
function ArrayCard({ arr, onClick }) {
  ...
  return (
    ...
    >
      ...
      {/* ── metrics ── */}
      <div style={{ padding: "8px 10px 8px", display: "flex", flexDirection: "column", gap: 5 }}>
        {/* name + dot */}
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <span style={{
            ...
            background: s.glow, boxShadow: `0 0 6px ${s.glow}`,
        ...
        <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between" }}>
          <div style={{ display: "flex", alignItems: "baseline", gap: 2 }}>
            <span style={{ fontSize: 19, fontWeight: 900, color: "#fff", lineHeight: 1 }}>
              {Number(arr.power || 0).toFixed(1)}
            </span>
            <span style={{ fontSize: 9, color: "rgba(255,255,255,0.35)" }}>W</span>
          </div>
          <span style={{
            fontSize: 7, fontWeight: 700, letterSpacing: "0.1em",
            textTransform: "uppercase", padding: "2px 6px", borderRadius: 99,
            background: arr.isLive ? `${s.glow}18` : "rgba(255,255,255,0.05)",
            border: arr.isLive ? `1px solid ${s.glow}40` : "1px solid rgba(255,255,255,0.08)",
        ...
        <div>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}>
            <span style={{ fontSize: 8, color: "rgba(255,255,255,0.3)", textTransform: "uppercase", letterSpacing: "0.08em" }}>Eff</span>
            <span style={{ fontSize: 9, fontWeight: 700, color: s.glow }}>{effPct}%</span>
          </div>
          <div style={{ height: 2, background: "rgba(255,255,255,0.07)", borderRadius: 99, overflow: "hidden" }}>
            <div style={{
              height: "100%", borderRadius: 99, width: `${effPct}%`,
        ...
        <div style={{
          ...
          padding: "2px 7px", borderRadius: 99, fontSize: 8,
          fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.07em",
          background: `${s.glow}14`, border: `1px solid ${s.glow}28`, color: s.glow,
...
export default function SolarFieldView() {
        ...
        const mapped = await Promise.all(
          devices.map(async (d, i) => {
            try {
              ...
              const { power, efficiency, irradiance } = history.length >= 2
  ...
  return (
    <div
      ...
      style={{ background: "#080c10", fontFamily: "'DM Mono', 'Fira Code', monospace" }}
    >
      {/* background grid */}
      <div
      ...
      <div className="absolute pointer-events-none" style={{
        top: -120, left: "30%", width: 500, height: 300,
        background: "radial-gradient(ellipse, rgba(34,217,122,0.07) 0%, transparent 70%)",
        filter: "blur(40px)",
      }} />
      <div className="absolute pointer-events-none" style={{
        bottom: 0, right: "10%", width: 400, height: 300,
        ...
        <div>
          <div className="flex items-center gap-3 mb-1">
            <div className="w-2 h-8 rounded-sm"
              style={{ background: "linear-gradient(180deg, #22d97a, #22d97a44)" }} />
        ...
        <div className="flex items-center gap-6 mt-1">
          ...
          <div className="flex items-center gap-4 text-[9px] text-white/30 uppercase tracking-widest">
            <span className="flex items-center gap-1.5">
              <span style={{ width: 6, height: 6, borderRadius: "50%", background: "#22d97a",
                display: "inline-block", boxShadow: "0 0 5px #22d97a" }} />
              Live
            </span>
            <span className="flex items-center gap-1.5">
      ...
      {!loading && (
        <div className="relative z-20 px-8 py-4 flex flex-wrap gap-6 border-b border-white/5">
          ...
          <Stat label="Total Power"   value={`${totalPower.toFixed(1)} W`}   color="#22d97a" />
      ...
      {loading && (
        <div className="relative z-20 flex flex-col items-center justify-center mt-32 gap-4">
          <div className="w-10 h-10 rounded-full border-2 animate-spin"
            style={{ borderColor: "rgba(255,255,255,0.1)", borderTopColor: "#22d97a" }} />
        </div>
      )}
      ...
      {!loading && (
        <div className="relative z-20 pb-8 flex justify-center">
          <div className="flex gap-6 px-6 py-3 rounded-full border border-white/8"
